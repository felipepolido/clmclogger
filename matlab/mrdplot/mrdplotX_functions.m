function mrdplotX_functions(task)
...
% December 1997
% Added filter button by Ben X, 2013
...
lptr = uicontrol('Parent',ptr, ...
 'Units','points', ...
 'Callback','mrdplotX_functions(''varlist'');',...
...
b = uicontrol('Parent',ptr, ...
 'Units','points', ...
 'Callback','mrdplotX_functions(''open'');',...
 'BackgroundColor',[0.5 0.3 0.8], ...
...
b = uicontrol('Parent',ptr, ...
 'Units','points', ...
 'Callback','if MRDS.zoom<500,MRDS.zoom=MRDS.zoom*1.5;end;mrdplotX_functions(''zoom'');',...
 'BackgroundColor',[0.9 0.9 0.5], ...
...
b = uicontrol('Parent',ptr, ...
 'Units','points', ...
 'Callback','if MRDS.zoom>1,MRDS.zoom=MRDS.zoom/1.5;end;mrdplotX_functions(''zoom'');',...
 'BackgroundColor',[0.9 0.9 0.5], ...
...
b = uicontrol('Parent',ptr, ...
 'Units','points', ...
 'Callback','mrdplotX_functions(''fast_open'');',...
 'BackgroundColor',[0.5 0.3 0.8], ...
 'Position',[9 77 55 23], ...
 'String','Fast Open', ...
 'Tag','Pushbutton1');
...
b = uicontrol('Parent',ptr, ...
 ...
 'String','d/dt', ...
 'Tag','Pushbutton1');
% Added Sclar operation
b = uicontrol('Parent',ptr, ...
 'Units','points', ...
 'Callback','MRDS.time=toc;MRDS.flag=9;',...
 'BackgroundColor',[0.4 0.4 0.5], ...
 'Position',[183 137 55 23], ...
 'String','Scalar', ...
 'Tag','Pushbutton1');
% Added Vector operation
b = uicontrol('Parent',ptr, ...
 'Units','points', ...
 'Callback','MRDS.time=toc;MRDS.flag=11;MRDS.var1=0;MRDS.var2=0;',...
 'BackgroundColor',[0.4 0.4 0.5], ...
 'Position',[183 107 55 23], ...
 'String','Vector', ...
 'Tag','Pushbutton1');
% Added LPF
b = uicontrol('Parent',ptr, ...
 'Units','points', ...
 'Callback','MRDS.time=toc;MRDS.flag=10;',...
 'BackgroundColor',[0.4 0.4 0.5], ...
 'Position',[183 77 55 23], ...
 'String','LPF', ...
 'Tag','Pushbutton1');
% Added FFT
b = uicontrol('Parent',ptr, ...
 'Units','points', ...
 'Callback','MRDS.time=toc;MRDS.flag=13;',...
 'BackgroundColor',[0.4 0.4 0.5], ...
 'Position',[183 47 55 23], ...
 'String','FFT', ...
 'Tag','Pushbutton1');


% Added Save data file
b = uicontrol('Parent',ptr, ...
 'Units','points', ...
 'Callback','mrdplotX_functions(''save_data'');',...
 'BackgroundColor',[0.4 0.4 0.5], ...
 'Position',[183 17 55 23], ...
 'String','Save Data', ...
 'Tag','Pushbutton1');
b = uicontrol('Parent',ptr, ...
 'Units','points', ...
...
b = uicontrol('Parent',ptr, ...
 'Units','points', ...
 'Callback','mrdplotX_functions(''clear_all'');',...
 'BackgroundColor',[0.5 0.3 0.8], ...
...
b = uicontrol('Parent',ptr, ...
 'Units','points', ...
 'Callback','mrdplotX_functions(''add_subplot'');',...
 'BackgroundColor',[0.5 0.3 0.8], ...
 'Position',[9 17 55 23], ...
 'String','Add Plot', ...
 'Tag','Pushbutton1');
b = uicontrol('Parent',ptr, ...
 'Units','points', ...
 'Callback','mrdplotX_functions(''delete_subplot'');',...
 'BackgroundColor',[0.5 0.3 0.8], ...
...
if ishandle(MRDS.fig),
 delete(MRDS.fig);
end;

ptr = figure(MRDS.fig);   % the window number is arbitrarily set to 451
MRDS.dptr = ptr;
...
set(ptr,'Units','points', ...
 'Color',[0.8 0.8 0.8], ...
 'Name','MRDPLOT-Data', ...
 'KeyPressFcn','mrdplotX_functions(''key_press'');',...
 'WindowButtonDownFcn','mrdplotX_functions(''cursor'');',...
 'NumberTitle','off', ...
 'Position',fig_size, ...
 'Tag','');
if exist('gestalt')==3,  % indicates MacOS
...
elseif (MRDS.flag == 6), %---Differentiation---
 if (toc - MRDS.time <= 6),
  ...
  prompt = {'Filter order:';'Filter cutoff frequency:';'Have delay?'};
  def = {2, 0.2, false};
  name =  'Differentiation Filter Options';
  formats(1,1) = struct('type','edit','format','integer','limits',[1 inf]);
  formats(2,1) = struct('type','edit','format','float','limits',[0 1]);
  formats(3,1).type ='check';
  formats(3,1).format = 'logical';
  formats(3,1).labelloc = 'topleft';
  for i = 1:3,
   formats(i,1).size = [200 22];
   formats(i,1).labelloc = 'topleft';
  end
  [reply,~] = inputsdlg(prompt, name, formats, def);
  order = reply{1};
  cutoff = reply{2};
  [b,a] = butter(order, cutoff);
  dvar = get(MRDS.lptr, 'value');
  ...
  x = x * MRDS.freq;
  if order ~= 0 & cutoff ~= 1,
   if reply{3}, % Have delay, use filter
    filt_x = filter(b, a, x);
   else,  % No Delay, use filtfilt
    filt_x = filtfilt(b, a, x);
   end;
  else
   filt_x = x;
  ...
  dname = sprintf('[d/dt][cut%s_delay%s]%s', num2str(cutoff), num2str(reply{3}), MRDS.vars(dvar).name);
...
elseif (MRDS.flag == 9), %---Scalar Operation---
 if (toc - MRDS.time <= 6),
  prompt = {'Scalar Value'; 'Options'};
  def = {1; 1};
  name =  'Scalar Operation Options';
  formats(1,1).type = 'edit';
  formats(1,1).format = 'float';
  formats(2,1).type = 'list';
  formats(2,1).style = 'popupmenu';
  formats(2,1).items = {'Add', 'Multiply', 'Power'};
  for i = 1:2,
   formats(i,1).size = [200 22];
   formats(i,1).labelloc = 'topleft';
  end
  ...
  dvar = get(MRDS.lptr, 'value');
  x = MRDS.data(:,dvar);
  switch reply{2}, % Choose which option
   case 1 % Add
    x = x + reply{1};
    dname = sprintf('[%s+]%s', num2str(reply{1}), MRDS.vars(dvar).name);
   case 2 % Multiply
    x = x*reply{1};
    dname = sprintf('[%s*]%s', num2str(reply{1}), MRDS.vars(dvar).name);
   case 3
  ...
  MRDS.data = [MRDS.data, x];
  MRDS.cols = MRDS.cols + 1;
  
  dunit = sprintf('%s', MRDS.vars(dvar).unit);
  dvar.name = dname;
  dvar.unit = dunit;
  MRDS.vars = [MRDS.vars, dvar];
  temp = sprintf('%s [%s]', dname, dunit);
...
elseif (MRDS.flag == 11), %---Vector Operation 1--
 if (toc - MRDS.time <= 6),
  MRDS.var1 = get(MRDS.lptr, 'value');
  MRDS.flag = 12;
 else,
...
elseif (MRDS.flag == 12), %---Vector Operation  2---
 MRDS.var2 = get(MRDS.lptr, 'value');
 MRDS.flag = 0;
 MRDS.time = toc;
 
 prompt = {'Scalar1'; 'Option'; 'Scalar2'; 'Option'; 'Vector Option'};
 def = {1; 1; 1; 1; 1};
 name =  'Vector Operation Options';
 formats(1,1).type = 'edit';
 formats(1,1).format = 'float';
 formats(2,1).type = 'list';
 formats(2,1).style = 'popupmenu';
 formats(2,1).items = {'Multiply', 'Power'};
 formats(3,1) = formats(1,1);
 formats(4,1) = formats(2,1);
 formats(5,1) = formats(2,1);
 formats(5,1).items = {'Add','Subtract','Multiply'};
 for i = 1:5,
  formats(i,1).size = [200 22];
  formats(i,1).labelloc = 'topleft';
 end
 [reply,~] = inputsdlg(prompt, name, formats, def);
 x1 = MRDS.data(:,MRDS.var1);
 x2 = MRDS.data(:,MRDS.var2);
 switch reply{2}, % Choose option for x1
  % case 1 % Add
  % x1 = x1 + reply{1};
  %str1 = sprintf('[%s+]%s', num2str(reply{1}), MRDS.vars(MRDS.var1).name);
  case 1 % Multiply
   x1 = x1*reply{1};
   str1 = sprintf('[%s*]%s', num2str(reply{1}), MRDS.vars(MRDS.var1).name);
  case 2 % Power
 ...
 switch reply{4}, % Choose option for x2
  %case 1 % Add
  %x2 = x2 + reply{3};
  %str2 = sprintf('[%s+]%s', num2str(reply{3}), MRDS.vars(MRDS.var2).name);
  case 1 % Multiply
   x2 = x2*reply{3};
   str2 = sprintf('[%s*]%s', num2str(reply{3}), MRDS.vars(MRDS.var2).name);
  case 2
 ...
 switch reply{5}, % Choose option for Vector operation
  case 1 % Add
   x = x1 + x2;
   dname = sprintf('%s[+]%s', str1, str2);
  case 2 % Subtract
   x = x1 - x2;
   dname = sprintf('%s[-]%s', str1, str2);
  case 3 % Multiply
...
elseif (MRDS.flag == 10), %---Low Pass Filter---
 if (toc - MRDS.time <= 6),
  % 		prompt = {'Filter order:','Filter cutoff frequency:'};
  % 		def = {'2','0.2'};
  % 		reply = inputdlg(prompt, 'Low Pass Filter Options', 1, def);
  % 		order = sscanf(char(reply(1)),'%f');
  % 		cutoff = sscanf(char(reply(2)), '%f
  prompt = {'Filter order:';'Filter cutoff frequency:';'Have delay?'};
  def = {2, 0.2, false};
  ...
  MRDS.data = [MRDS.data, filt_x];
  MRDS.cols = MRDS.cols + 1;
  dname = sprintf('[LPF_cut%s_%sdelay]%s', num2str(cutoff), num2str(reply{3}),MRDS.vars(dvar).name);
  dunit = sprintf('%s', MRDS.vars(dvar).unit);
  dvar.name = dname;
  dvar.unit = dunit;
  MRDS.vars = [MRDS.vars, dvar];
...
elseif (MRDS.flag == 13), %---FFT---
 if (toc - MRDS.time <= 6),
  ...
  Fs = MRDS.freq;                    % Sampling frequency
  T = 1/Fs;
  prompt = {'From'; 'To'; 'Normalized'};
  def = {0, MRDS.rows/MRDS.freq, false};
  name =  'Signal range in seconds';
  for i = 1:2,
   formats(i,1).type = 'edit';
   formats(i,1).format = 'float';
   formats(i,1).limits = [0 MRDS.rows/MRDS.freq];
   formats(i,1).size = [200 22];
   formats(i,1).labelloc = 'topleft';
  end
  formats(3,1).type = 'check';
  ...
  formats(3,1).format = 'logical';
  formats(3,1).size = [200 22];
  formats(3,1).labelloc = 'topleft';
  [reply,~] = inputsdlg(prompt, name, formats, def);  
  
  ind1 = reply{1}*MRDS.freq;
  ind2 = reply{2}*MRDS.freq; 

  x = x(ceil(ind1+1): floor(ind2),1); % Resize x
  L = length(x); % Range size
  NFFT = 2^nextpow2(L); % Next power of 2 from length of y
  f = Fs/2*linspace(0,1,NFFT/2+1);
  X = fft(x, NFFT)/L;
  if reply{3} % Normalize
   if max(abs(X)) % Maximum is non zero
    X = bsxfun(@times, X, 1./max(abs(X)));
   end
  end
  figure;
  plot(f,2*abs(X(1:NFFT/2+1,:)));
  grid on
  tl = sprintf('FFT of %s, from %s(s) to %s(s)', MRDS.vars(dvar).name, ...
   num2str(reply{1}), num2str(reply{2}));
  a = title(tl); 
  set(a,'Interpreter','none','FontSize',MRDS.fontsize);
  xlabel('Frequency (Hz)','FontSize',MRDS.fontsize);
  ylabel('Magnitude','FontSize',MRDS.fontsize);
 end;
else %---Normal action---
 MRDS.time=toc;
 MRDS.flag=1;
...
%------------------------------------------------------------------------------------------
function save_data
% save modified data to mrdformat
global MRDS;

%function mrdplot_gen(data,names,units,freq,fname)
mrdplotX_gen(MRDS.data, extractfield(MRDS.vars, 'name'), extractfield(MRDS.vars, 'unit'), MRDS.freq, MRDS.fname);
